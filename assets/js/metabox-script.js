(function ($) {
    ...
    var FAQMetabox = {
        ...
        init: function () {
            ...
            // ✅ NEW: Expose reload function globally
            window.faqMetaboxReload = this.reloadMetabox.bind(this);
        },
        ...
        bindEvents: function () {
            ...
            $(document).on('click', '.faq-item-move-up', function (e) {
                e.preventDefault();
                var $card = $(this).closest('.faq-item-card');
                self.moveUp($card);
            });

            $(document).on('click', '.faq-item-move-down', function (e) {
                e.preventDefault();
                var $card = $(this).closest('.faq-item-card');
                self.moveDown($card);
            });
            ...
            if ($.fn.sortable && $('#faq-sortable-list').length) {
                $('#faq-sortable-list').sortable({
                    ...
                    start: function (event, ui) {
                        ...
                        ui.item.addClass('faq-item-dragging');
                    },
                    stop: function (event, ui) {
                        ui.item.removeClass('faq-item-dragging');
                    },
                    update: function (event, ui) {
                        self.updateIndices();
                        self.updateHiddenField();
                        self.markAsChanged();
                        self.showMessage('success', 'Order changed. Click "Update" to save.');
                    }
                });
            }
        },
        ...
        // ✅ NEW: Move item up
        moveUp: function ($card) {
            var $prev = $card.prev('.faq-item-card');
            if ($prev.length) {
                $card.insertBefore($prev);
                this.updateIndices();
                this.updateHiddenField();
                this.markAsChanged();
                this.showMessage('success', 'Item moved up. Click "Update" to save.');
            }
        },

        // ✅ NEW: Move item down
        moveDown: function ($card) {
            var $next = $card.next('.faq-item-card');
            if ($next.length) {
                $card.insertAfter($next);
                this.updateIndices();
                this.updateHiddenField();
                this.markAsChanged();
                this.showMessage('success', 'Item moved down. Click "Update" to save.');
            }
        },

        // ✅ NEW: Reload metabox (called after generating FAQs)
        reloadMetabox: function () {
            var self = this;
            var schemaData = $('#faq-schema-data').val();
            ...
            try {
                ...
                if (schema && schema.mainEntity && schema.mainEntity.length > 0) {
                    this.rebuildFromJSON(schema);
                    this.showMessage('success', 'FAQs loaded from generator.');
                }
            } catch (e) {
        ...
        updateFAQ: function (index, question, answer) {
            ...
            $card.attr('data-question', question);
            $card.attr('data-answer', answer);
        ...
        rebuildFromJSON: function (schema) {
            ...
            // If empty state exists, remove it and create structure
            if ($('.faq-empty-state').length) {
                $('.faq-empty-state').remove();
        ...
        createFAQCard: function (index, question, answer) {
            ...
            return $('<div class="faq-item-card" data-index="' + index + '" data-question="' + this.escapeHtml(question) + '" data-answer="' + this.escapeHtml(answer) + '">' +
                '<div class="faq-item-drag" title="Drag to reorder"><span class="dashicons dashicons-menu"></span></div>' +
                ...
                '<button type="button" class="faq-item-btn faq-item-move-up" title="Move Up"><span class="dashicons dashicons-arrow-up-alt2"></span></button>' +
                '<button type="button" class="faq-item-btn faq-item-move-down" title="Move Down"><span class="dashicons dashicons-arrow-down-alt2"></span></button>' +
